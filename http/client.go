...
// Request is request.
//
// ex) response, err := http.Request("http://127.0.0.1:10000/test/id-01", net_http.MethodGet, map[string][]string{"header-1": {"value-1"}}, "", 10, "", "")
func Request(url, method string, header map[string][]string, body string, timeout time.Duration, username, password string) (Response, error) {
