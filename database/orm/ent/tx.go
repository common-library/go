...
func (tx *Tx) init() {
	tx.IssueForEnt = NewIssueForEntClient(tx.config)
	tx.RepositoryForEnt = NewRepositoryForEntClient(tx.config)
	tx.Table01ForEnt = NewTable01ForEntClient(tx.config)
	tx.UserForEnt = NewUserForEntClient(tx.config)
}
...
// applies a query, for example: IssueForEnt.QueryXXX(), the query will be executed
